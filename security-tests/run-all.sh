...
API="${API:-https://api-vaulted.casacam.net/api}"
EMAIL="${EMAIL:-owner@test.com}"
PASSWORD="${PASSWORD:-Test1234!Secure}"

# Load MFA_SECRET from .env if present (generated by get-mfa-secret.sh)
...
check "Referrer-Policy present"            "referrer-policy"           "${HEADERS,,}"
...
RETURNED_TENANT=$(echo "$R" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('data',{}).get('tenantId',''))" 2>/dev/null || echo "")
if [[ "$R" == *"othertenant"* || "$RETURNED_TENANT" == "othertenant" ]]; then
  fail_msg "Mass assignment: tenantId accepted from client" "response=$R"
elif [[ "$STATUS" == "400" || "$STATUS" == "401" || "$STATUS" == "403" ]]; then
  pass_msg "Mass assignment: tenantId rejected by validation/auth" "status=$STATUS"
else
  pass_msg "Mass assignment: tenantId not echoed from client" "returnedTenant=${RETURNED_TENANT:-none}"
fi
...
R=$(curl -s -o /dev/null -w "%{http_code}" \
  ...
  -H "Authorization: Bearer $TOKEN")
if [[ "$R" == "500" ]]; then
  fail_msg "NoSQL operator in query param caused 500 error" "status=$R"
else
  pass_msg "NoSQL operator in query param handled without 500" "status=$R"
fi
...
if [[ "$R" == "500" ]]; then
  fail_msg "Path traversal in filename caused server error" "status=$R"
else
  pass_msg "Path traversal in filename handled without 500" "status=$R"
fi
rm -f /tmp/normal.jpg
...
if [[ -z "$ACAO" ]]; then
  pass_msg "Unauthorized origin gets no ACAO header"
elif [[ "$ACAO" == *"evil-attacker.com"* ]]; then
  fail_msg "CORS allows evil-attacker.com" "$ACAO"
else
  pass_msg "CORS did not reflect evil origin" "$ACAO"
fi
...
# 6.3 Origin: null must never be reflected
R=$(curl -sI -X OPTIONS "$API/users" \
  -H "Origin: null" \
  -H "Access-Control-Request-Method: GET")
ACAO=$(echo "$R" | grep -i "access-control-allow-origin" | tr -d '\r' || echo "")
if echo "$ACAO" | grep -qi "null"; then
  fail_msg "CORS reflects Origin: null" "$ACAO"
else
  pass_msg "CORS does not allow Origin: null" "$ACAO"
fi

# 6.4 credentials=true with wildcard is a credential theft risk
R=$(curl -sI -X OPTIONS "$API/users" \
  -H "Origin: https://evil-attacker.com" \
  -H "Access-Control-Request-Method: GET")
ACAO=$(echo "$R" | grep -i "access-control-allow-origin" | tr -d '\r' || echo "")
ACAC=$(echo "$R" | grep -i "access-control-allow-credentials" | tr -d '\r' || echo "")
if echo "$ACAO" | grep -q "\*" && echo "$ACAC" | grep -qi "true"; then
  fail_msg "CORS uses wildcard origin with credentials=true" "$ACAO $ACAC"
else
  pass_msg "CORS avoids wildcard origin with credentials=true" "$ACAO $ACAC"
fi
